...
const statusColorMap = {
  ...
  active:"bg-bllue-500/20 text-blue-800 border-blue-800/5",
    ...
    cell: ({ row }) => (
      <div className="flex flex-col gap-y-1">
        ...
        <div className="flex items-center gap-x-2">
          ...
          {generatedAvatar({
            seed: row.original.agent.name || "User",
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge 
        variant="outline"
        className={cn("text-muted-foreground [&>svg]:size-4", statusColorMap[row.original.status as keyof typeof statusColorMap])}>
